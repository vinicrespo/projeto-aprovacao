...
void main() {
  ...
  // Starts at ~600 blocks (≈2px each) so 0.01 is barely visible;
  // exponential scale keeps low end gentle, high end obviously pixelated
  if (u_crackle_intensity > 0.001) {
    float blocks = floor(exp(mix(log(600.0), log(6.0), u_crackle_intensity)));
